...
2. Press `Cmd/Ctrl+Shift+P` and run `Data Convert: Convert Format...` (data formats) or `Data Convert: Encode / Decode...` (encodings).
...
- `Query String → JSON` accepts a whole URL — everything after `?` is used. Repeated keys collapse into an array, and `JSON → Query String` expands arrays back into repeated keys. Bracket syntax is supported both ways, so `a[b]=1` becomes a nested object.
- `Text → Unicode Escape` escapes non-ASCII characters and backslashes, with emoji escaped one code unit at a time as surrogate pairs. Backslashes have to be escaped for the transform to be reversible — otherwise a literal `\u4e2d` already in your text would decode back as a character. Decoding accepts `\\`, `\uXXXX` and `\u{XXXXX}`.
