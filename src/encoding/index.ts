 ...
 *
 * 反斜杠必须先转义，否则原文里本来就有的 `\u4e2d` 字面量会在解码时被误当成转义序列，
 * 往返就不再等价。
 */
...
/** `\\` 与 `\uXXXX` 放在同一遍处理，避免先还原反斜杠又被当成转义前缀 */
const fromUnicodeEscape: Transform = (text) =>
  text.replace(/\\\\|\\u\{([0-9a-fA-F]+)\}|\\u([0-9a-fA-F]{4})/g, (match, braced, plain) =>
    match === '\\\\' ? '\\' : String.fromCodePoint(Number.parseInt(braced ?? plain, 16))
  );
