...
2. `Cmd/Ctrl+Shift+P` 执行 `Data Converter: Convert Format...`（数据格式）或 `Data Converter: Encode / Decode...`（编码）；
...
- `Query String → JSON` 可以直接粘整条 URL，取 `?` 之后的部分；重复键收成数组，`JSON → Query String` 再把数组展开成重复键；
- `Text → Unicode Escape` 只转义非 ASCII 字符，emoji 按代理对逐个码元转义；解码同时接受 `\uXXXX` 与 `\u{XXXXX}`；
